...
exports.lockFreeTrialUsers = functions
  .region('asia-south1')
  .runWith({ timeoutSeconds: 540, memory: '1GB' })
  .https.onRequest(async (req, res) => {
    if (req.method !== 'POST') {
    ...
    const WRITE_BATCH_LIMIT = 200;

    let lastKey = null;
    let pendingUpdates = {};
    let pendingUpdateUsers = 0;
    ...
    const flushPendingUpdates = async () => {
      if (pendingUpdateUsers === 0) return;
      await USERS.update(pendingUpdates);
      stats.writeBatches += 1;
      pendingUpdates = {};
      pendingUpdateUsers = 0;
    };

    functions.logger.info('Starting global free-trial enforcement job');

    while (true) {
      let query = USERS.orderByKey().limitToFirst(FETCH_LIMIT);
      ...
      for (const child of children) {
        ...
        const rawCountry = resolveCountry(data);
        const normalizedCountry = rawCountry ? normalizeCountry(rawCountry) : null;

        if (!normalizedCountry) {
          stats.skippedMissingCountry += 1;
        }

        const countryKey = normalizedCountry || 'unknown';
        stats.countryCounts[countryKey] = (stats.countryCounts[countryKey] || 0) + 1;
        ...
        if (data.isPremium) {
          stats.skippedPremium += 1;
          continue;
        }

        if (data.freeTrialCompleted) {
          stats.skippedCompleted += 1;
          continue;
        }

        pendingUpdates[`${uid}/hasUsedFreeTrial`] = true;
        pendingUpdates[`${uid}/freeTrialCompleted`] = true;
        pendingUpdates[`${uid}/freeTrialExpiry`] = 0;
        pendingUpdates[`${uid}/freeTrialStartedAt`] = 0;

        stats.locked += 1;
        pendingUpdateUsers += 1;

        if (pendingUpdateUsers >= WRITE_BATCH_LIMIT) {
          await flushPendingUpdates();
        }
      }

      if (children.length < FETCH_LIMIT) break;
    }

    await flushPendingUpdates();

    functions.logger.info('Completed global free-trial enforcement job', { stats });
